...
function App() {
  ...
  const [comments, setComments] = useState([]); // 책 상세 댓글용
  ...
  const [formData, setFormData] = useState({ title: '', author: '', image_url: '', image_url_2: '', region: 'NY', meeting_date: '', season: '1' });
    ...
    <div className="min-h-screen bg-[#F9F7F2] text-stone-800 font-['Noto_Serif_KR'] selection:bg-stone-200 overflow-x-hidden">
      ...
      {activeTab === 'CALENDAR' && (
        <div className="pt-32 max-w-7xl mx-auto px-4 md:px-6 pb-40 text-left animate-in fade-in duration-700">
            <h3 className="text-2xl md:text-3xl font-black text-stone-900 tracking-tight flex items-center gap-3 mb-12 text-left">
        ...
        <div className="pt-24 max-w-[1500px] mx-auto px-4 md:px-8 pb-20 animate-in fade-in duration-700 min-h-screen flex flex-col font-['Noto_Serif_KR']">
          ...
          <div className="flex flex-col lg:flex-row gap-0 border border-stone-200 bg-white shadow-xl rounded-sm overflow-hidden flex-1 min-h-[650px]">
            <aside className="lg:w-72 border-r border-stone-100 flex flex-col bg-stone-50/30">
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar -webkit-overflow-scrolling-touch text-left text-left">
                {essays.map((essay) => (
            ...
            <main className="flex-1 bg-white p-6 md:p-12 lg:p-16 overflow-y-auto custom-scrollbar text-left relative text-left text-left">
              {selectedEssay ? (
                <article className="animate-in fade-in slide-in-from-right-4 duration-500 max-w-4xl mx-auto">
                  <header className="mb-10 text-left text-left text-left">
                    ...
                    <div className="flex items-center gap-5 py-5 border-y border-stone-100 font-bold text-stone-800">Author: {selectedEssay.author}<span className="text-[11px] text-stone-400 font-medium italic ml-auto">{new Date(selectedEssay.created_at).toLocaleDateString()}</span></div>
                  </header>
        ...
        <div className="pt-40 max-w-2xl mx-auto px-6 pb-40 animate-in fade-in duration-1000 text-center text-center">
            ...
            <div className="space-y-4 text-left text-left">
                {[
                  ...
                  <div key={idx} onClick={() => window.open(form.url)} className="p-6 md:p-10 bg-white border border-stone-200 shadow-md hover:-translate-y-1 hover:shadow-2xl transition-all cursor-pointer flex justify-between items-center group rounded-sm text-left text-left">
      ...
      {isZoomed && selectedPost && (
        ...
        >
          <img 
            src={currentImageIndex === 0 ? selectedPost.image_url : selectedPost.image_url_2} 
        ...
        <div className="fixed inset-0 bg-stone-900/90 backdrop-blur-sm flex items-center justify-center p-0 md:p-10 z-[300]" onClick={() => setSelectedPost(null)}>
          <div className="bg-white w-full max-w-7xl h-full md:h-[90vh] overflow-hidden flex flex-col md:flex-row shadow-2xl animate-in zoom-in duration-500" onClick={e => e.stopPropagation()}>
            ...
            <div className="h-[50%] md:h-full md:w-[55%] bg-[#FBFBF9] flex flex-col items-center justify-center p-6 md:p-10 border-r border-stone-100 relative text-center text-center">
                <div className="relative group flex flex-col items-center w-full h-full justify-center">
                  <div className="relative cursor-zoom-in group/img" onClick={() => setIsZoomed(true)}>
                    <img 
                      src={currentImageIndex === 0 ? selectedPost.image_url : (selectedPost.image_url_2 || selectedPost.image_url)} 
                  ...
                  {/* 슬라이드 버튼 */}
                  {selectedPost.image_url_2 && (
                    <div className="flex gap-10 mt-6 mb-2">
                      <button onClick={() => setCurrentImageIndex(0)} className={`p-2 rounded-full transition-all ${currentImageIndex === 0 ? 'text-[#722F37] bg-stone-200' : 'text-stone-300'}`}><ChevronLeft size={30}/></button>
                      <button onClick={() => setCurrentImageIndex(1)} className={`p-2 rounded-full transition-all ${currentImageIndex === 1 ? 'text-[#722F37] bg-stone-200' : 'text-stone-300'}`}><ChevronRight size={30}/></button>
                    </div>
                  )}
            ...
            <div className="h-[50%] md:h-full md:w-[45%] p-8 md:p-12 flex flex-col bg-white relative text-left text-left text-left">
              ...
              <div className="flex-1 overflow-y-auto space-y-6 mb-8 pr-4 custom-scrollbar -webkit-overflow-scrolling-touch text-left text-left text-left text-left text-left">
      ...
      {selectedEvent && (
        <div className="fixed inset-0 bg-stone-900/95 flex items-center justify-center p-4 z-[500]" onClick={() => setSelectedEvent(null)}>
          <div className="bg-white rounded-sm w-full max-w-md shadow-2xl relative text-left overflow-hidden animate-in zoom-in duration-300 text-left text-left" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-10 text-left text-left">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-stone-900/70 backdrop-blur-sm flex items-center justify-center p-4 z-[400]" onClick={() => setIsModalOpen(false)}>
          <div className="bg-white p-10 w-full max-w-lg shadow-2xl relative text-left rounded-sm text-left text-left" onClick={e => e.stopPropagation()}>
            <button onClick={() => setIsModalOpen(false)} className="absolute top-6 right-6 text-stone-400"><X size={20}/></button>
            <h3 className="text-[12px] font-black tracking-[0.2em] text-stone-400 mb-8 text-center uppercase border-b border-stone-100 pb-4 text-center text-center text-center">New Book Entry</h3>
            <form onSubmit={async (e) => { e.preventDefault(); const {error}=await supabase.from('reviews').insert([formData]); if(!error){ setIsModalOpen(false); setFormData({title:'',author:'',image_url:'',image_url_2:'',region:'NY',meeting_date:'',season:'1'}); fetchPosts(); } }} className="space-y-6">
              <div className="flex gap-8 justify-center mb-6 text-center">{['NY', 'NJ'].map(r => (<button key={r} type="button" onClick={() => setFormData({...formData, region: r})} className={`text-[12px] font-black tracking-widest transition-all ${formData.region === r ? 'text-[#722F37] border-b-2 border-[#722F37] pb-1' : 'text-stone-300'}`}>{r}</button>))}</div>
              <div className="grid grid-cols-2 gap-4"><div className="space-y-1 text-left text-left text-left text-left"><label className="text-[10px] font-black text-stone-400 uppercase tracking-widest">Season</label><input type="number" className="w-full bg-transparent border-b border-stone-200 py-1 outline-none font-bold text-base text-left" value={formData.season} onChange={e => setFormData({...formData, season: e.target.value})} /></div><div className="space-y-1 text-left text-left text-left text-left"><label className="text-[10px] font-black text-stone-400 uppercase tracking-widest">Meeting Date</label><input type="date" required className="w-full bg-transparent border-b border-stone-200 py-1 outline-none font-bold text-sm text-left" value={formData.meeting_date} onChange={e => setFormData({...formData, meeting_date: e.target.value})} /></div></div>
              ...
              <button type="submit" className="w-full bg-stone-900 text-white py-4 text-[12px] font-black tracking-widest uppercase hover:bg-stone-800 transition-all">Register</button>
            </form>
          </div>
        </div>
      )}
      ...
      {isEventModalOpen && (
        <div className="fixed inset-0 bg-stone-900/70 backdrop-blur-sm flex items-center justify-center p-4 z-[450]" onClick={() => setIsEventModalOpen(false)}>
          <div className="bg-white p-8 w-full max-w-lg shadow-2xl relative text-left rounded-sm text-left text-left text-left" onClick={e => e.stopPropagation()}>
            <button onClick={() => setIsEventModalOpen(false)} className="absolute top-6 right-6 text-stone-400"><X size={20}/></button>
            <h3 className="text-[12px] font-black tracking-[0.2em] text-stone-400 mb-8 text-center uppercase border-b border-stone-100 pb-4 text-center text-center text-center text-center">Schedule Entry</h3>
            <form onSubmit={async (e) => { e.preventDefault(); const {error}=await supabase.from('events').insert([newEvent]); if(!error){ setIsEventModalOpen(false); setNewEvent({title:'',start:'',description:'',region:'NY',image_url:''}); fetchEvents(); } }} className="space-y-6">
              <div className="flex gap-6 justify-center mb-6 text-center text-center text-center">
                {['NY', 'NJ', 'EVENT'].map(r => (<button key={r} type="button" onClick={() => setNewEvent({...newEvent, region: r})} className={`text-[11px] font-black tracking-widest px-3 py-1 transition-all ${newEvent.region === r ? 'text-[#722F37] border-b-2 border-[#722F37]' : 'text-stone-300'}`}>{r === 'NY' ? '🍎 NY' : r === 'NJ' ? '🌳 NJ' : '👥 EVENT'}</button>))}
              </div>
              <input type="text" placeholder="Title" required className="w-full bg-transparent border-b border-stone-200 py-2 outline-none font-black text-base text-left" value={newEvent.title} onChange={e => setNewEvent({...newEvent, title: e.target.value})} /><textarea placeholder="Description" className="w-full bg-[#FBFBF9] p-4 h-32 outline-none font-medium text-[14px] border border-stone-200 text-left text-left text-left text-left text-left" value={newEvent.description} onChange={e => setNewEvent({...newEvent, description: e.target.value})} /><input type="text" placeholder="Image Link" className="w-full bg-transparent border-b border-stone-200 py-2 outline-none font-medium text-sm text-left" value={newEvent.image_url} onChange={e => setNewEvent({...newEvent, image_url: e.target.value})} /><button type="submit" className="w-full bg-stone-900 text-white py-4 text-[12px] font-black tracking-widest uppercase hover:bg-stone-800 transition-all">Save</button>
            </form>
          </div>
        </div>
      )}
      ...
      {isEssayModalOpen && (
        <div className="fixed inset-0 bg-stone-900/80 backdrop-blur-sm flex items-center justify-center p-4 z-[400]" onClick={() => setIsEssayModalOpen(false)}>
          <div className="bg-white p-8 md:p-10 w-full max-w-2xl shadow-2xl relative text-left rounded-sm animate-in zoom-in duration-300 text-left text-left text-left text-left" onClick={e => e.stopPropagation()}>
            ...
            <div className="text-center mb-8 text-center text-center text-center"><PenTool size={24} className="mx-auto text-[#722F37] mb-3" /><h3 className="text-lg font-black tracking-[0.2em] text-stone-800 uppercase text-center text-center text-center text-center text-center">New Essay Entry</h3></div>
            <form onSubmit={async (e) => { e.preventDefault(); const {error} = await supabase.from('essays').insert([essayForm]); if(!error){ alert("등록되었습니다."); setEssayForm({title:'', book_title:'', author:'', content:''}); setIsEssayModalOpen(false); fetchEssays(); } }} className="space-y-5 text-left text-left text-left text-left">
      ...
      <footer className="max-w-7xl mx-auto px-6 py-24 border-t border-stone-200 text-center font-['Noto_Serif_KR']">
        ...
        <div className="space-y-4 mb-12 text-center text-center text-center text-center">
          <p className="text-[12px] text-stone-600 font-black tracking-[0.2em] uppercase text-center text-center text-center text-center text-center">Copyright Disclaimer</p>
          <p className="text-[13px] md:text-[14px] text-stone-500 leading-relaxed max-w-2xl mx-auto font-light italic text-center text-center text-center text-center text-center">
          ...
          <div className="flex items-center justify-center gap-2 mt-6 py-2 px-4 bg-stone-100/50 w-fit mx-auto rounded-full text-center text-center text-center"><Mail size={14} className="text-stone-400" /><a href="mailto:yaboo99mung@gmail.com" className="text-[13px] text-[#722F37] font-black">yaboo99mung@gmail.com</a></div>
        </div>
        <p className="text-[9px] tracking-[0.4em] text-stone-300 uppercase font-black text-center text-center text-center text-center text-center">© 2025 NJ·NY Book Club. All rights reserved.</p>
      </footer>
